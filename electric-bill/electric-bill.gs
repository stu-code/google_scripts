...
// ============================================================================
const CONFIG = {
  SENDER_EMAIL: 'dukeenergypaperlessbilling@paperless.dukeenergy.com',
  SUBJECT_CONTAINS: 'Energy statement is ready',
  // Matches "Due Date: July 6, 2026" or the AP-style abbreviated form Duke also
  // uses for longer month names, e.g. "Due Date: Sept. 8, 2026". The "\.?" makes
  // the period optional so both forms match.
  DATE_REGEX: /DUE DATE\s*:\s*([A-Z]+)\.?\s+(\d{1,2}),\s*(\d{4})/,
  EVENT_NAME: 'Electric bill',
...
// Keyed by the first 3 letters, so this matches both full names ("JULY") and
// abbreviations ("SEPT", "SEP") without needing an entry for every variant.
...
function parseDueDate(bodyUpper) {
  ...
  const month = MONTH_PREFIXES[match[1].substring(0, 3)];
...
function createCalendarEvent(dueDate) {
  ...
  event.setDescription('Google Apps Script. BillBot: electric-bill');
